...
<br />
    <div class="portlet">
		...
		<?php for($i=0;$i<sizeof($id);$i++)
				print '<tr><td width="100px"></td><td  width="90px"><a href="index.php?components=cms&action=show_approve_cr&id='.$id[$i].'" >'.str_pad($id[$i],5,"0",STR_PAD_LEFT).'</a></td><td  width="90px"><a href="index.php?components=cms&action=show_approve_cr&id='.$id[$i].'" >'.$cr_date[$i].'</a></td><td><a href="index.php?components=cms&action=show_approve_cr&id='.$id[$i].'" >'.$title[$i].'</a></td><td width="50px"></td></tr>';
		?>
		</table>
		</div>
      </div>
